...
class CalculateBox
{
    ...
    public $box_max_size = array(false, false, false);
    public $debug = false;
    ...
    public function findBoxSizeByMaxSize()
    {
        $this->items = $this->sortItems($this->items);
        ...
        foreach ($this->items as $item_id => $item) {
            ...
            foreach ($orientations as $orientation) {
                $this->addToDebug("Rotating item by edge: " . $orientation);
                $rotated_item = $this->rotateByEdge($item, $orientation);
                $this->addToDebug("Rotated item: " . $this->debugObject($rotated_item));

                foreach ($orientations as $add_to_edge) {
                    $this->addToDebug("Rotating box by edge: " . $add_to_edge);
                    $temp_box = $this->addItemToBox($this->box, $rotated_item, $add_to_edge);
                    $this->addToDebug("Temporary box received: " . $this->debugObject($temp_box));

                    if ($temp_box->getWidth() <= $this->box_max_size[0] &&
                        $temp_box->getHeight() <= $this->box_max_size[1] &&
                        $temp_box->getLength() <= $this->box_max_size[2])
                    {
                        $this->addToDebug("Box is smaller then: " . $this->debugObject($this->box_max_size));
                        $this->box = $temp_box;
                        $this->addToDebug('***************** END OF ITEM ADD *****************');
                        $is_placed = true;
                        break 2;
            ...
            if (!$is_placed) {
                $this->addToDebug("Failed to insert item");
                $this->addToDebug('***************** END OF ITEM ADD *****************');
                return false;
            }
        }
    ...
    {
        $this->addToDebug('Adding item to box edge: ' . $add_to_edge);
